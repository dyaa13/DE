...
function y8GenCircleArea() {
  const L=state.level,t=L==='starter'?randInt(1,6):L==='core'?randInt(1,10):randInt(1,14);
  if(t===1){const r=pick([2,3,4,5,6,8,10,12]);return q('circleArea',`A circle has radius ${r} cm. Circumference = ?π cm. Enter the coefficient of π.`,2*r,'Circumference = 2πr.');}
  if(t===2){const r=pick([2,3,4,5,6,7,8,9,10]);return q('circleArea',`A circle has radius ${r} cm. Area = ?π cm². Enter the coefficient of π.`,r*r,'Area = πr².');}
  if(t===3){const d=pick([6,8,10,12,14,16,18,20]);return q('circleArea',`A circle has diameter ${d} cm. Radius = ? cm`,d/2,'Radius = diameter ÷ 2.');}
  if(t===4){const r=pick([2,3,4,5,6,7,8,9,10]);return q('circleArea',`A circle has area ${r*r}π cm². Radius = ? cm`,r,'If A = πr², the coefficient of π is r².');}
  if(t===5){const d=pick([4,6,8,10,12,14,16,18,20]);return q('circleArea',`A circle has diameter ${d} cm. Circumference = ?π cm. Enter the coefficient of π.`,d,'Circumference = πd.');}
  if(t===6){const d=pick([4,6,8,10,12,14,16,18,20]),r=d/2;return q('circleArea',`A circle has diameter ${d} cm. Area = ?π cm². Enter the coefficient of π.`,r*r,'Find the radius first, then use A = πr².');}
  if(t===7){const r=pick([2,4,6,8,10,12]);return q('circleArea',`A semicircle has radius ${r} cm. Area = ?π cm². Enter the coefficient of π.`,r*r/2,'A semicircle has half the area of a full circle.');}
  if(t===8){const r=pick([2,4,6,8,10,12]);return q('circleArea',`A quarter circle has radius ${r} cm. Area = ?π cm². Enter the coefficient of π.`,r*r/4,'A quarter circle has one quarter of the full circle area.');}
  if(t===9){const r=pick([2,3,4,5,6,8,10]);return q('circleArea',`A semicircle has radius ${r} cm. Its perimeter is ${r}π + ? cm. Find the non-π part.`,2*r,'Perimeter = curved length πr + diameter 2r.');}
  if(t===10){const r=pick([4,6,8,10,12]);return q('circleArea',`A quarter circle has radius ${r} cm. Its perimeter is ${r/2}π + ? cm. Find the non-π part.`,2*r,'Perimeter = quarter-circle arc + two radii.');}
  if(t===11){const c=pick([8,10,12,14,16,18,20]),r=c/2;return q('circleArea',`A circle has circumference ${c}π cm. Area = ?π cm². Enter the coefficient of π.`,r*r,'From C = 2πr, first find r = C coefficient ÷ 2.');}
  if(t===12){const r=pick([3,4,5,6,8,10]),side=2*r;return q('circleArea',`A square of side ${side} cm contains a circle of radius ${r} cm. Area outside the circle = ${side*side} − ?π cm². Find the coefficient of π.`,r*r,'Circle area = πr².');}
  if(t===13){const r1=pick([2,3,4,5]),r2=pick([6,7,8,9,10]);return q('circleArea',`Two circles have radii ${r1} cm and ${r2} cm. Difference in area = ?π cm². Enter the coefficient of π.`,r2*r2-r1*r1,'Subtract the area coefficients r₂² − r₁².');}
  const r=pick([4,6,8,10,12]),half=r*r/2,quarter=r*r/4;return q('circleArea',`A semicircle and a quarter circle both have radius ${r} cm. Their combined area = ?π cm². Enter the coefficient of π.`,half+quarter,'Add one half and one quarter of πr².');
}

YEAR_BANKS[8] = {
  "circleArea": y8GenCircleArea,
