...
function y9GenCircleSectors() {
  const L=state.level,t=L==='starter'?randInt(1,6):L==='core'?randInt(1,11):randInt(1,16);
  const cases=[[6,30,1,3],[8,45,2,8],[12,45,3,18],[6,60,2,6],[12,60,4,24],[6,90,3,9],[8,90,4,16],[10,90,5,25],[12,90,6,36],[6,120,4,12],[9,120,6,27],[12,120,8,48],[8,180,8,32],[10,180,10,50],[8,270,12,48]];
  if(t===1){const r=pick([3,4,5,6,8,10,12]);return q('circleSectors',`A circle has radius ${r} cm. Circumference = ?π cm. Enter the coefficient of π.`,2*r,'Circumference = 2πr.');}
  if(t===2){const r=pick([3,4,5,6,7,8,9,10]);return q('circleSectors',`A circle has radius ${r} cm. Area = ?π cm². Enter the coefficient of π.`,r*r,'Area = πr².');}
  if(t===3){const [r,angle,arc]=pick(cases);return q('circleSectors',`A circle has radius ${r} cm and central angle ${angle}°. Arc length = ?π cm. Enter the coefficient of π.`,arc,'Arc length = angle/360 × 2πr.');}
  if(t===4){const [r,angle,,sector]=pick(cases);return q('circleSectors',`A sector has radius ${r} cm and angle ${angle}°. Area = ?π cm². Enter the coefficient of π.`,sector,'Sector area = angle/360 × πr².');}
  if(t===5){const R=pick([5,6,7,8,9,10]),r=pick([2,3,4]);if(r>=R)return y9GenCircleSectors();return q('circleSectors',`A ring has outer radius ${R} cm and inner radius ${r} cm. Area = ?π cm². Enter the coefficient of π.`,R*R-r*r,'Ring area = π(R² − r²).');}
  if(t===6){const r=pick([3,4,5,6,8,10]),side=2*r;return q('circleSectors',`A square of side ${side} cm contains a circle of radius ${r} cm. Area outside the circle = ${side*side} − ?π cm². Find the coefficient of π.`,r*r,'Subtract the circle area from the square area.');}
  if(t===7){const [r,angle,arc]=pick(cases.filter(x=>x[1]<=180));return q('circleSectors',`An arc of a circle with radius ${r} cm has length ${arc}π cm. The central angle = ?°`,angle,'Use arc/full circumference = angle/360.');}
  if(t===8){const [r,angle,,sector]=pick(cases.filter(x=>x[1]<=180));return q('circleSectors',`A sector of radius ${r} cm has area ${sector}π cm². Central angle = ?°`,angle,'Use sector area/full circle area = angle/360.');}
  if(t===9){const [r,angle,arc]=pick(cases.filter(x=>x[1]<180));return q('circleSectors',`A circle has radius ${r} cm. A ${angle}° minor arc has length ${arc}π cm. The major arc length = ?π cm. Enter the coefficient of π.`,2*r-arc,'Major arc = full circumference − minor arc.');}
  if(t===10){const [r,angle,arc]=pick(cases.filter(x=>x[1]<=180));return q('circleSectors',`A sector has radius ${r} cm and angle ${angle}°. Its perimeter is ${arc}π + ? cm. Find the non-π part.`,2*r,'Sector perimeter = arc length + two radii.');}
  if(t===11){const [r,angle,,sector]=pick(cases.filter(x=>x[1]<=180));return q('circleSectors',`A sector has radius ${r} cm and angle ${angle}°. Fraction of the full circle area = ?/360. Enter the numerator.`,angle,'The sector is angle/360 of the full circle.');}
  if(t===12){const [r,angle,arc]=pick(cases.filter(x=>x[1]<=120));const turns=pick([2,3,4]);return q('circleSectors',`${turns} identical arcs each have radius ${r} cm and angle ${angle}°. Total arc length = ?π cm. Enter the coefficient of π.`,turns*arc,'Find one arc length, then multiply by the number of arcs.');}
  if(t===13){const R=pick([6,7,8,9,10,12]),r=pick([2,3,4,5]);if(r>=R)return y9GenCircleSectors();return q('circleSectors',`A ring has area ${R*R-r*r}π cm² and outer radius ${R} cm. Inner radius = ? cm`,r,'Use R² − r² from the coefficient of π.');}
  if(t===14){const [r,angle,,sector]=pick(cases.filter(x=>x[1]<=180));const full=r*r;return q('circleSectors',`A ${angle}° sector of radius ${r} cm has area ${sector}π cm². The rest of the circle has area ?π cm². Enter the coefficient of π.`,full-sector,'Subtract the sector area from the full circle area.');}
  if(t===15){const [r,angle,arc]=pick(cases.filter(x=>x[1]<=180));return q('circleSectors',`For radius ${r} cm, a ${angle}° arc has length ${arc}π cm. If the angle doubles, the new arc length = ?π cm. Enter the coefficient of π.`,2*arc,'For a fixed radius, arc length is directly proportional to the central angle.');}
  const [r,angle,arc,sector]=pick(cases.filter(x=>x[1]<=120));return q('circleSectors',`A sector has radius ${r} cm and angle ${angle}°. Find (sector-area coefficient) − (arc-length coefficient).`,sector-arc,'Use the coefficients from sector area and arc length, then subtract.');
}

YEAR_BANKS[9] = {
  "circleSectors": y9GenCircleSectors,
