...
function generateQuestion(options = {}) {
  ...
  const requestedSkill = options.skill || null;
  let validFallback = null;

  for (let attempt = 0; attempt < 80; attempt++) {
    const activeSkills = requestedSkill
      && config.skills.includes(requestedSkill)
      && getActiveSkills().includes(requestedSkill)
      ? [requestedSkill]
      : getActiveSkills();
  ...
  const body = hasChoices
    ? `
      <div class="print-question-text">
        ${formatMathHtml(question.choicePrompt || question.text)}
      </div>
      <ol class="print-options">
        ${question.choiceOptions
          .map(option => `<li>${formatMathHtml(option)}</li>`)
          .join('')}
      </ol>
    `
    : `<div class="print-question-text">${formatMathHtml(question.text)}</div>`;
...
function buildPrintableAnswerHtml(question, number) {
  let answer = displayCorrect(question);

  if (
    Array.isArray(question.choiceOptions)
    && question.choiceOptions.length > 0
  ) {
    const choiceNumber = Number(question.answer);
    const choiceText = question.choiceOptions[choiceNumber - 1];

    answer = choiceText != null
  ...
  return `
    <div class="print-answer-item">
      <b>${number}.</b>
      <span>${formatMathHtml(answer)}</span>
    </div>
  `;
}

function generatePrintableQuestionSet(questionCount = 50) {
  const activeSkills = getActiveSkills();

  if (activeSkills.length === 0) {
    throw new Error('Select at least one skill before exporting questions.');
  }

  const skillOrder = [];

  while (skillOrder.length < questionCount) {
    const cycle = shuffleCopy(activeSkills);
    skillOrder.push(...cycle);
  }
  ...
  try {
    ...
    for (let index = 0; index < questionCount; index++) {
      const preferredSkill = skillOrder[index];
      let chosen = null;
      ...
      if (!chosen) {
        throw new Error(`Unable to generate question ${index + 1}.`);
      }
...
function exportSelectedQuestionsToPdf() {
  ...
  if (getActiveSkills().length === 0) {
    alert('Select at least one skill before exporting the 50-question PDF.');
    return;
  }
  ...
  if (!printWindow) {
    alert('The PDF window was blocked. Allow pop-ups for this page and try again.');
    return;
  }
  ...
  printWindow.document.write(`
    ...
    <head>
      <meta charset="UTF-8">
      <title>Preparing DYAA 50 Questions...</title>
    </head>
    <body style="font-family:Arial,sans-serif;padding:30px">
      <h2>Preparing 50 questions...</h2>
  ...
  exportPdfBtn.textContent = 'Preparing 50 Questions...';
  ...
  try {
    ...
    const fileTitle = `DYAA_Year_${state.year}_Selected_50_Questions`;

    const questionHtml = questions
      .map((question, index) => buildPrintableQuestionHtml(question, index + 1))
      .join('');
    const answerHtml = questions
      .map((question, index) => buildPrintableAnswerHtml(question, index + 1))
      .join('');
      ...
      <head>
        ...
        <style>
          ...
          * {
          ...
          .print-answer-page h1 {
            margin: 0 0 2mm;
            color: #173b64;
            font-size: 22pt;
          }

          .print-answer-subtitle {
            margin-bottom: 6mm;
            color: #64748b;
            font-size: 10pt;
          }
          ...
          @media screen {
            body {
              ...
              margin: 0 auto;
              padding: 12mm;
              box-shadow: 0 0 20px rgba(15, 39, 66, 0.16);
            }
          }
      ...
      <body>
        <header class="print-header">
          <div class="print-brand-row">
            <div class="print-brand">DYAA</div>
            <div class="print-title">Year ${state.year} - Selected 50 Questions</div>
          </div>
          <div class="print-meta">
            <div><b>Name:</b> <span class="print-name-line">${safeStudent}</span></div>
        ...
        <main class="print-question-grid">
          ${questionHtml}
        </main>

        <section class="print-answer-page">
          <h1>Answer Key</h1>
          <div class="print-answer-subtitle">
            Year ${state.year} - ${escapeHtml(selectedLevel)} - ${escapeHtml(config.title)}
          </div>
          <div class="print-answer-grid">
            ${answerHtml}
          </div>
          <div class="print-footer">
            DYAA Education - Together We Learn, Together We Grow
          </div>
        </section>
    ...
    printWindow.close();
    alert(`Could not prepare the 50-question PDF: ${error.message || String(error)}`);
  } finally {
...
function setControlsForGame(active) {
  ...
  exportPdfBtn.classList.toggle('hidden', active);
...
exportPdfBtn.addEventListener('click', exportSelectedQuestionsToPdf);
