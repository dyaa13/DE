...
function y7GenCircleBasics() {
  const L=state.level,t=L==='starter'?randInt(1,5):L==='core'?randInt(1,8):randInt(1,10);
  if(t===1){const r=pick([2,3,4,5,6,7,8,9,10,12]);return q('circleBasics',`A circle has radius ${r} cm. Diameter = ? cm`,2*r,'Diameter = 2 × radius.');}
  if(t===2){const d=pick([6,8,10,12,14,16,18,20,24]);return q('circleBasics',`A circle has diameter ${d} cm. Radius = ? cm`,d/2,'Radius = diameter ÷ 2.');}
  if(t===3){const r=pick([2,3,4,5,6,7,8,10,12]);return q('circleBasics',`A circle has radius ${r} cm. Circumference = ?π cm. Enter the coefficient of π.`,2*r,'Circumference = 2πr.');}
  if(t===4){const d=pick([4,6,8,10,12,14,16,18,20]);return q('circleBasics',`A circle has diameter ${d} cm. Circumference = ?π cm. Enter the coefficient of π.`,d,'Circumference = πd.');}
  if(t===5){const r=pick([2,3,4,5,6,7,8,9,10]);return q('circleBasics',`A circle has circumference ${2*r}π cm. Radius = ? cm`,r,'Use C = 2πr.');}
  if(t===6){const d=pick([6,8,10,12,14,16,18,20]);return q('circleBasics',`A circle has circumference ${d}π cm. Diameter = ? cm`,d,'Use C = πd.');}
  if(t===7){const r=pick([2,3,4,5,6,8,10,12]);return q('circleBasics',`A semicircle has radius ${r} cm. Its curved length = ?π cm. Enter the coefficient of π.`,r,'The curved length is half of 2πr, so it is πr.');}
  if(t===8){const item=pick([[1,'radius'],[2,'diameter'],[3,'circumference']]);return q('circleBasics',`Which measure goes from the centre of a circle to its edge? Enter 1 for radius, 2 for diameter, or 3 for circumference.`,1,'A radius joins the centre to the circumference.');}
  if(t===9){const r=pick([2,3,4,5,6,8,10]),turns=pick([2,3,4,5]);return q('circleBasics',`A wheel has radius ${r} cm and makes ${turns} full turns. Distance travelled = ?π cm. Enter the coefficient of π.`,2*r*turns,'One turn covers one circumference, 2πr.');}
  const r1=pick([2,3,4,5,6]),r2=pick([7,8,9,10,12]);return q('circleBasics',`Circle A has radius ${r1} cm and Circle B has radius ${r2} cm. Their circumferences differ by ?π cm. Enter the coefficient of π.`,2*(r2-r1),'Subtract the circumference coefficients: 2r₂ − 2r₁.');
}

YEAR_BANKS[7] = {
  "circleBasics": y7GenCircleBasics,
